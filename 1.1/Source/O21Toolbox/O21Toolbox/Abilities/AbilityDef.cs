...
namespace O21Toolbox.Abilities
{
    public class AbilityDef : Def
    {
        ...
        public Job GetJob(AbilityTargetCategory cat, LocalTargetInfo target)
        {
            switch (cat)
            {
                case AbilityTargetCategory.TargetSelf:
                    {
                        return new Job(AbilityDefOf.O21_CastAbilitySelf, target);
                    }
                case AbilityTargetCategory.TargetAoE:
                    {
                        return new Job(AbilityDefOf.O21_CastAbilityVerb, target);
                    }
        ...
        public virtual string GetAoEDesc()
        {
            ...
            if (def != null)
            {
                if (def.TargetAoEProperties != null)
                {
                    ...
                    s.AppendLine(StringsToTranslate.O21_AU_AoEProperties);
                    if (def.TargetAoEProperties.targetClass == typeof(Pawn))
                    {
                        s.AppendLine("\t" + StringsToTranslate.O21_AU_TargetClass + StringsToTranslate.O21_AU_TargetClass);
                    }
                    else
                    {
                        s.AppendLine("\t" + StringsToTranslate.O21_AU_TargetClass + def.TargetAoEProperties.targetClass.ToString().CapitalizeFirst());
                    }
                    s.AppendLine("\t" + "Range".Translate() + ": " + def.TargetAoEProperties.range);
                    s.AppendLine("\t" + StringsToTranslate.O21_AU_TargetClass + def.TargetAoEProperties.friendlyFire);
                    s.AppendLine("\t" + StringsToTranslate.O21_AU_AoEMaxTargets + def.TargetAoEProperties.maxTargets);
                    if (def.TargetAoEProperties.startsFromCaster)
                    {
                        s.AppendLine("\t" + StringsToTranslate.O21_AU_AoEStartsFromCaster);
                    }
                    result = s.ToString();
                }
            }
        ...
        public string GetBasics()
        {
            ...
            {
                var s = new StringBuilder();
                s.AppendLine(StringsToTranslate.O21_AU_Cooldown + def.SecondsToRecharge.ToString("N0") + " " +
                             "SecondsLower".Translate());
                switch (def.AbilityTargetCategory)
                {
                    case AbilityTargetCategory.TargetAoE:
                        s.AppendLine(StringsToTranslate.O21_AU_Type + StringsToTranslate.O21_AU_TargetAoE);
                        break;
                    case AbilityTargetCategory.TargetSelf:
                        s.AppendLine(StringsToTranslate.O21_AU_Type + StringsToTranslate.O21_AU_TargetSelf);
                        break;
                    case AbilityTargetCategory.TargetThing:
                        s.AppendLine(StringsToTranslate.O21_AU_Type + StringsToTranslate.O21_AU_TargetThing);
                        break;
                    case AbilityTargetCategory.TargetLocation:
                        s.AppendLine(StringsToTranslate.O21_AU_Type + StringsToTranslate.O21_AU_TargetLocation);
                        break;
                }
                if (def.tooltipShowProjectileDamage)
                {
                    if (def.defaultProjectile != null)
                    {
                        if (def.defaultProjectile.projectile != null)
                        {
                            ...
                            {
                                s.AppendLine("Damage".Translate() + ": " + def.defaultProjectile.projectile.GetDamageAmount(1f));
                                s.AppendLine("Damage".Translate() + " " + StringsToTranslate.O21_AU_Type + def.defaultProjectile.projectile.damageDef.LabelCap);
                            }
                        }
                    }
                }
                if (def.tooltipShowExtraDamages)
                {
                    if (def.extraDamages != null && def.extraDamages.Count > 0)
                    {
                        ...
                        {
                            s.AppendLine(StringsToTranslate.O21_AU_Extra + " " + "Damage".Translate() + ": " + def.extraDamages[0].damage);
                            s.AppendLine(StringsToTranslate.O21_AU_Extra + " " + "Damage".Translate() + " " + StringsToTranslate.O21_AU_Type + def.extraDamages[0].damageDef.LabelCap);
                        }
                        else
                        {
                            s.AppendLine(StringsToTranslate.O21_AU_Extra + " " + "Damage".Translate() + ": ");
                            foreach (var extraDam in def.extraDamages)
                            {
                                s.AppendLine("\t" + StringsToTranslate.O21_AU_Extra + " " + "Damage".Translate() + " " + StringsToTranslate.O21_AU_Type + extraDam.damageDef.LabelCap);
                                s.AppendLine("\t" + StringsToTranslate.O21_AU_Extra + " " + "Damage".Translate() + ": " + extraDam.damage);
                            }
                        }
                    }
                }
                if (def.tooltipShowMentalStatesToApply)
                {
                    if (def.mentalStatesToApply != null && def.mentalStatesToApply.Count > 0)
                    {
                        ...
                        {
                            s.AppendLine(StringsToTranslate.O21_AU_MentalStateChance + ": " + def.mentalStatesToApply[0].mentalStateDef.LabelCap + " " + def.mentalStatesToApply[0].applyChance.ToStringPercent());
                        }
                        else
                        {
                            s.AppendLine(StringsToTranslate.O21_AU_MentalStateChance);
                ...
                if (def.tooltipShowHediffsToApply)
                {
                    if (def.hediffsToApply != null && def.hediffsToApply.Count > 0)
                        ...
                        {
                            s.AppendLine(StringsToTranslate.O21_AU_EffectChance + def.hediffsToApply[0].hediffDef.LabelCap +
                                         " " + def.hediffsToApply[0].applyChance.ToStringPercent());
                        }
                        else
                        {
                            s.AppendLine(StringsToTranslate.O21_AU_EffectChance);
                    ...
                    {
                        s.AppendLine(StringsToTranslate.O21_AU_BurstShotCount + " " + def.burstShotCount);
                    }
                }
