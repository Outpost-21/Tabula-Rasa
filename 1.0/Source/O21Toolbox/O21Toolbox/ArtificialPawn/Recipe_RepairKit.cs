...
namespace O21Toolbox.ArtificialPawn
{
    public class Recipe_RepairKit : RecipeWorker
    {
        public override IEnumerable<BodyPartRecord> GetPartsToApplyOn(Pawn pawn, RecipeDef recipe)
        {
            ...
            if (pawn.def.HasModExtension<ArtificialPawnProperties>())
            {
                if (
                    ...
                    hediffs.Any(hediff => hediff.def.HasModExtension<ArtificialPawnProperties>() && hediff.CurStage.becomeVisible == true))
                    yield return null;
            }
            else
            {
        ...
        {
            //Remove Coolant Loss
            Hediff coolantLoss = pawn.health.hediffSet.GetFirstHediffOfDef(pawn.def.GetModExtension<EnergyHediffs>().coolantLoss);
            if (coolantLoss != null)
            ...
            if (pawn.def.HasModExtension<ArtificialPawnProperties>())
            {
                ...
                {
                    if (hediff is Hediff_MissingPart || hediff is Hediff_Injury || hediff.def.HasModExtension<ArtificialPawnProperties>())
