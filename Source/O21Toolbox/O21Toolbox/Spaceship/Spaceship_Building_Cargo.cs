...
{
    public class Spaceship_Building_Cargo : Spaceship_Building, ITrader
    {
        ...
        // ===================== Setup work =====================
        public void InitializeParameters_Cargo(Faction faction, int hitPoints, int landingDuration, SpaceshipDef spaceshipDef, SpaceshipKind spaceshipKind)
        {
            base.InitializeParameters(faction, hitPoints, landingDuration, spaceshipDef, spaceshipKind);
        }
        ...
        public string TraderName
        {
            get
            {
                return this.Faction.Name.ToString() + " " + this.thisSpaceshipDef.label;
            }
        }
            ...
            foreach (IntVec3 cell in this.OccupiedRect().Cells)
            {
                foreach (Thing thing in cell.GetThingList(this.Map))
                {
                    if ((TradeUtility.EverPlayerSellable(thing.def))
                        ...
                        && (thingsWillingToBuy.Contains(thing) == false))
                    {
                        thingsWillingToBuy.Add(thing);
                    }
                }
