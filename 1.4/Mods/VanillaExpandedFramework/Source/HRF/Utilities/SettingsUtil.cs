...
namespace HRF
{
    public static class SettingsUtil
    {
        ...
        public static void SettingsDropdown(this Listing_Standard listing, string name, string explanation, ref HRFSettingsPage value, float width)
        {
            float curHeight = listing.CurHeight;
            Rect rect = listing.GetRect(Text.LineHeight + listing.verticalSpacing);
            Text.Font = GameFont.Small;
            ...
            if (Widgets.ButtonText(rect2, value.ToString().Replace("_", " "), true, true, true))
            {
                ...
                List<HRFSettingsPage> enumValues = Enum.GetValues(typeof(HRFSettingsPage)).Cast<HRFSettingsPage>().ToList();
                foreach (HRFSettingsPage enumValue in enumValues)
                {
                    list.Add(new FloatMenuOption(enumValue.ToString().Replace("_", " "), delegate ()
                    {
                        HRFMod.mod.currentPage = enumValue;
                    }));
                }
