    ...
    public class Comp_AdvancedHatcher : ThingComp
    {
        ...
        public void Hatch()
        {
            for (int i = 0; i < parent.stackCount; i++)
            {
                ...
                if (chosenDef != null)
                {
                    PawnGenerationRequest request = new PawnGenerationRequest(chosenDef, hatcheeFaction, PawnGenerationContext.NonPlayer, -1, forceGenerateNewPawn: true, newborn: true);
