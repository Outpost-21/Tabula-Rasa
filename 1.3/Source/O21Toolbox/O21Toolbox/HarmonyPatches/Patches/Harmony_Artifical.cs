...
namespace O21Toolbox.HarmonyPatches
{
    ...
    public class Patch_ImmunityHandler_DiseaseContractChanceFactor
    {
        ...
        public static void Postfix(ImmunityHandler __instance, float __result)
        {
            if (__result > 0f)
            {
                if (__instance.pawn is Pawn && __instance.pawn.def.HasModExtension<DefModExt_ArtificialPawn>())
                {
                    __result = 0f;
                }
            }
    ...
    //{
    //    [HarmonyPrefix]
    //    public static void Prefix(StunHandler __instance, DamageInfo dinfo, Thing ___parent)
    //    {
    ...
    public class Patch_StunHandler_AffectedByEMP
    {
        ...
        public static void Postfix(ref bool __result, ref StunHandler __instance, Thing ___parent)
        {
            ...
            {
                Pawn pawn = (Pawn)___parent;
                if (pawn != null && pawn.def.HasModExtension<DefModExt_ArtificialPawn>())
                {
    ...
    public class Patch_SkillRecord_Interval
    {
        ...
        public static bool Prefix(SkillRecord __instance)
        {
            ...
            DefModExt_ArtificialPawn modExt;
            bool flag = (modExt = pawn.def.GetModExtension<DefModExt_ArtificialPawn>()) != null && modExt.noSkillLoss;
            return !flag;
        }
    }
    ...
    public class Patch_ThingDef_IsIngestible
    {
        ...
        {
            if (__instance.HasModExtension<DefModExt_ArtificialPawn>())
            {
                if (__instance.GetModExtension<DefModExt_ArtificialPawn>().corpseEdible)
                {
                    return true;
                }
                else
