    ...
    public class Verb_Deflected : Verb_Shoot
    {
        ...
        public bool TryCastShot_V1Vanilla_Modified()
        {
            ...
            if (verbProps.forcedMissRadius > 0.5f)
            {
                ...
                if (num > 0.5f)
                {
                    ...
                    if (num2 > 0)
                    {
                        ...
                        projectile.Launch(caster, currentTarget, c, projectileHitFlags, false, EquipmentSource); //TODO
                        return true;
                    }
                }
            }
            ...
            if (!Rand.Chance(shotReport.AimOnTargetChance_IgnoringPosture))
            {
                ...
                projectile.Launch(caster, currentTarget, shootLine.Dest, projectileHitFlags2, false, EquipmentSource); //TODO
                return true;
            }
            if (this.currentTarget.Thing != null && this.currentTarget.Thing.def.category == ThingCategory.Pawn && !Rand.Chance(shotReport.PassCoverChance))
            {
                ...
                if (currentTarget.Thing != null && currentTarget.Thing.def.category == ThingCategory.Pawn)
                {
                    ...
                    projectile.Launch(caster, currentTarget, randomCoverToMissInto, projectileHitFlags5, false, EquipmentSource);
                    return true;
                }
            }
            else
            {
                if (!Rand.Chance(shotReport.PassCoverChance))
                {
                    ...
                    if (this.currentTarget.Thing != null && this.currentTarget.Thing.def.category == ThingCategory.Pawn)
                    {
                        ...
                        projectile.Launch(caster, drawPos, randomCoverToMissInto, this.currentTarget,
                            projectileHitFlags3, false, EquipmentSource, targetCoverDef);
                        return true;
                    }
                }
                if (DebugViewSettings.drawShooting)
                ...
                {
                    projectile.Launch(caster, drawPos, this.currentTarget, this.currentTarget, projectileHitFlags4, false,
                        EquipmentSource, targetCoverDef);
                }
                else
                {
                    projectile.Launch(caster, drawPos, shootLine.Dest, this.currentTarget, projectileHitFlags4, false,
                        EquipmentSource, targetCoverDef);
                }
                result = true;
            }
