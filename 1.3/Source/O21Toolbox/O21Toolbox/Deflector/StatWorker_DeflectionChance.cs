...
namespace O21Toolbox.Deflector
{
    public class StatWorker_DeflectionChance : StatWorker
    {
        ...
        {
            if (GetDeflector(req) is Comp_Deflector compDeflector)
                return new Comp_Deflector.DeflectionChanceCalculator(compDeflector, fixedRandSeed: true).Calculate();
            return 0f;
        }

        private Comp_Deflector GetDeflector(StatRequest req)
        {
            return req.Thing is Pawn pawn ? GetDeflector(pawn) : null;
        }

        private Comp_Deflector GetDeflector(Pawn pawn)
        {
            if (pawn.equipment is Pawn_EquipmentTracker equipmentTracker)
            {
                ...
                {
                    if (equipment?.GetComp<Comp_Deflector>() is Comp_Deflector compDeflector)
        ...
        public override string GetExplanationUnfinalized(StatRequest req, ToStringNumberSense numberSense)
        {
            ...
            else
            {
                ...
                {
                    var infixDeclaredType = compDeflector.GetType().GetMethod(nameof(Comp_Deflector.DeflectionChance_InFix)).DeclaringType;
        ...
        {
            return thing is Pawn pawn && GetDeflector(pawn) is Comp_Deflector compDeflector &&
