...
namespace O21Toolbox
{
    public static class EnhancedSettings
    {
		...
		public static void SettingsDropdown<T>(this Listing_Standard listing, string name, string explanation, ref O21ToolboxSettingsPage value, float width, string tooltip = null)
		{
			float curHeight = listing.CurHeight;
			Rect rect = listing.GetRect(Text.LineHeight + listing.verticalSpacing);
			Text.Font = GameFont.Small;
			...
			if (Widgets.ButtonText(rect2, value.ToString().Replace("_", " "), true, true, true))
			{
				...
				List<O21ToolboxSettingsPage> enumValues = Enum.GetValues(typeof(O21ToolboxSettingsPage)).Cast<O21ToolboxSettingsPage>().ToList();
				foreach (O21ToolboxSettingsPage enumValue in enumValues)
				{
					list.Add(new FloatMenuOption(enumValue.ToString().Replace("_", " "), delegate ()
					{
						O21ToolboxMod.mod.currentPage = enumValue;
					}));
				}
