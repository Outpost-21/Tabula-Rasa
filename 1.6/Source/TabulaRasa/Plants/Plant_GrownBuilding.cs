    ...
    public class Plant_GrownBuilding : Plant
	{
		...
		public override void TickLong()
		{
			...
			if (PlantUtility.GrowthSeasonNow(Map, def))
			{
				...
				{
					Map.mapDrawer.MapMeshDirty(Position, (ulong)MapMeshFlagDefOf.Things);
				}
			}
			...
			if (Dying)
			{
				...
				if (Destroyed)
				{
					if (isCrop && def.plant.Harvestable && MessagesRepeatAvoider.MessageShowAllowed("MessagePlantDiedOfRot-" + def.defName, 240f))
					{
						...
						Messages.Message(key.Translate(GetCustomLabelNoCount(includeHp: false)), new TargetInfo(Position, map2), MessageTypeDefOf.NegativeEvent);
					}
					return;
				}
			}
