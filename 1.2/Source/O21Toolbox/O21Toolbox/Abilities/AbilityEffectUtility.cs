        ...
        public static PawnSummoned SpawnPawn(SpawnThings spawnables, Faction faction, Pawn caster, IntVec3 positionHeld)
        {
            ...
            if (faction != null && faction != Faction.OfPlayer)
            {
                ...
                if (newPawn.Map.mapPawns.SpawnedPawnsInFaction(faction).Any(p => p != newPawn))
                {
                    ...
                    var p2 = (Pawn)GenClosest.ClosestThing_Global(newPawn.Position, newPawn.Map.mapPawns.SpawnedPawnsInFaction(faction), 99999f, validator);
                    lord = p2.GetLord();
                }
                if (lord == null)
                {
                    ...
                    foreach (var hediffs in localApplyHediffs)
                    {
                        ...
                        if (success)
                        {
                            ...
                            MoteMaker.ThrowText(victim.DrawPos, victim.Map,
                                hediffs.hediffDef.LabelCap + ": " + StringsToTranslate.O21_AU_CastSuccess, -1f);
                        }
                        else
                        {
                            MoteMaker.ThrowText(victim.DrawPos, victim.Map, StringsToTranslate.O21_AU_CastFailure, -1f);
                        }
                    }
                }
            }
            ...
            if (localApplyMentalStates != null)
            {
                if (localApplyMentalStates.Count > 0)
                {
                    foreach (var mentalStateGiver in localApplyMentalStates)
                    {
                        ...
                        string str = localAbilityDef.LabelCap + " (" + caster.LabelShort + ")";
                        ...
                        {
                            victim.Drawer.Notify_DebugAffected();
                            MoteMaker.ThrowText(victim.DrawPos, victim.Map, mentalStateGiver.mentalStateDef.LabelCap + ": " + StringsToTranslate.O21_AU_CastSuccess, -1f);
                        }
                        else
                        {
                            MoteMaker.ThrowText(victim.DrawPos, victim.Map, mentalStateGiver.mentalStateDef.LabelCap + ": " + StringsToTranslate.O21_AU_CastFailure, -1f);
                        }
                    }
                }
            }
